...
V2b changes vs v1:
  - Keep overnight gap (first bar of day NOT removed)
  - dist="normal" instead of "t"
  - KEEP GJR: o=1 (leverage term preserved)

Usage:
    python scripts/garch_10min_v2b.py
"""
...
RESULTS_DIR = BASE_DIR / "results" / "garch_10min_v2b"
...
def load_and_prepare(ticker: str):
    ...
    V2b: overnight gap is NOT removed (first bar of day kept).
    """
    ...
    # Log returns (global shift — first bar of each day contains overnight gap)
    df["log_return"] = np.log(df["close"] / df["close"].shift(1))

    # V2b: do NOT remove first bar — keep overnight gap
...
def process_ticker(ticker: str):
    """Fit GJR-GARCH(1,1,1) normal on train 10-min returns, filter through test."""
    try:
    ...
    # ── Fit GJR-GARCH(1,1,1) normal on train ──
    try:
        m_train = arch_model(
...
def main():
    print("=" * 60)
    print("  GARCH-10min V2b: GJR-GARCH(1,1,1) normal, with overnight")
    print("=" * 60)
    ...
    for h in HORIZONS:
        ...
        out_path = RESULTS_DIR / f"garch_10min_v2b_h{h}.parquet"
    ...
    print(f"{'='*60}")
    print(f"{'Model':<30} {'H=1':>8} {'H=5':>8} {'H=22':>8}")
    print(f"{'─'*60}")
    print(f"{'HAR-J':<30} {'0.3052':>8} {'0.4243':>8} {'0.4669':>8}")
    print(f"{'GARCH-GJR (daily)':<30} {'1.8017':>8} {'1.0514':>8} {'0.7320':>8}")
    print(f"{'GARCH-10min v1 (GJR-t)':<30} {'0.2987':>8} {'0.4147':>8} {'0.4927':>8}")

    g10 = "GARCH-10min v2b (GJR-norm)"
    vals = [f"{qlike_results.get(h, float('nan')):.4f}" for h in HORIZONS]
    print(f"{g10:<30} {vals[0]:>8} {vals[1]:>8} {vals[2]:>8}")
    print("=" * 60)
