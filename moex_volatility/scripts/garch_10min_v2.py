...
V2 changes vs v1:
  ...
  - Simple GARCH(1,1): o=0 (no leverage/asymmetry term)

Usage:
    python scripts/garch_10min_v2.py
"""
...
RESULTS_DIR = BASE_DIR / "results" / "garch_10min_v2"
...
def load_and_prepare(ticker: str):
    ...
    V2: overnight gap is NOT removed (first bar of day kept).
    """
    ...
    # V2: do NOT remove first bar — keep overnight gap
...
def process_ticker(ticker: str):
    """Fit GARCH(1,1) normal on train 10-min returns, filter through test."""
    try:
...
def main():
    print("=" * 60)
    print("  GARCH-10min V2: GARCH(1,1) normal, with overnight")
    print("=" * 60)
    ...
    for h in HORIZONS:
        ...
        out_path = RESULTS_DIR / f"garch_10min_v2_h{h}.parquet"
    ...
    print(f"{'='*60}")
    print(f"{'Model':<25} {'H=1':>8} {'H=5':>8} {'H=22':>8}")
    print(f"{'─'*60}")
    print(f"{'HAR-J':<25} {'0.3052':>8} {'0.4243':>8} {'0.4669':>8}")
    print(f"{'GARCH-GJR (daily)':<25} {'1.8017':>8} {'1.0514':>8} {'0.7320':>8}")
    print(f"{'GARCH-10min v1 (GJR-t)':<25} {'0.2987':>8} {'0.4147':>8} {'0.4927':>8}")

    g10 = "GARCH-10min v2"
    vals = [f"{qlike_results.get(h, float('nan')):.4f}" for h in HORIZONS]
    print(f"{g10:<25} {vals[0]:>8} {vals[1]:>8} {vals[2]:>8}")
    print("=" * 60)
