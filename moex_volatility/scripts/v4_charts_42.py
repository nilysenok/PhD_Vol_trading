...
def fig_4_6():
    ...
    data = {
        ...
        "D": [3.213, 3.042, 1.843, 2.449, 1.666, 3.303],
    }
...
def fig_4_7():
    approaches = ["A", "B", "C", "D"]
    exposure   = [15.4, 3.7, 13.8, 11.9]
    turnover   = [2.3, 2.2, 4.6, 2.1]
    colors     = COLORS_ABCD
...
def fig_4_8():
    ...
    delta = np.array([
        [ 2.1,  31.9, 22.7],   # Контртренд
        [83.6,  53.2, 84.7],   # Тренд
        [12.1,  15.3, 10.6],   # Диапазон
    ])
...
def fig_4_9():
    ...
    daily_A = [2.556, 2.541, 1.051, 1.262, 1.662, 2.729]
    daily_D = [3.213, 3.042, 1.843, 2.449, 1.666, 3.303]

    hourly_A = [0.305, 1.791, 0.741, 1.268, 1.553, 0.477]
    hourly_D = [2.212, 2.622, 1.569, 2.030, 2.409, 1.173]
    ...
    ax1.set_ylim(0, max(max(daily_A), max(daily_D)) * 1.15)
    ...
    ax2.set_ylim(0, max(max(hourly_A), max(hourly_D)) * 1.15)
