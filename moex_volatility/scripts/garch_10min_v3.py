...
V3: same as v1, but overnight gap is NOT removed.
  - GJR-GARCH(1,1,1) with Student-t  (same as v1)
  - Keep first bar of each day (overnight gap included)

Usage:
    python scripts/garch_10min_v3.py
"""
...
RESULTS_DIR = BASE_DIR / "results" / "garch_10min_v3"
...
def load_and_prepare(ticker: str):
    ...
    V3: overnight gap is NOT removed (first bar of day kept).
    """
    ...
    # V3: do NOT remove first bar — keep overnight gap
...
def process_ticker(ticker: str):
    """Fit GJR-GARCH(1,1,1) t-dist on train 10-min returns, filter through test."""
    try:
...
def main():
    print("=" * 60)
    print("  GARCH-10min V3: GJR-GARCH(1,1,1) t, WITH overnight")
    print("=" * 60)
    ...
    for h in HORIZONS:
        ...
        out_path = RESULTS_DIR / f"garch_10min_v3_h{h}.parquet"
    ...
    print(f"{'GARCH-10min v1 (no overnight)':<30} {'0.2987':>8} {'0.4147':>8} {'0.4927':>8}")

    g10 = "GARCH-10min v3 (w/ overnight)"
